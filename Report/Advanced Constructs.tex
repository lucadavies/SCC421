    ...
    \subsection{Static Code Analysis}
        ...
        \label{subsubsec:sharpLab}
            ...
            \begin{itemize}
                ...
                \item Increment Adv uses 2 add operations where plain uses 3. Because ++/-- causes a value COPY that has been optimised into existence by default, despite requiring more instructions
                \item Compound assignment IS 1-to-1
                \item Foreach Adv markedly longer. Requires try-finally to ensure enumerator is disposed
            \end{itemize}
            
            Java
            \begin{itemize}
                \item Ternary identical EXCEPT: plain return INSIDE if like in source, Adv always runs to the closing Return instruction.
                \item Lambda shorter in place, but has more code in `bootstrap function' elsewhere
                \item Increment Adv is better..! Yay! (maybe) JVM has an increment instruction (with variable number to increment by) so it compiles 1-to-1 to an increment instruction.
                \item Compound assignment uses same instruction as above, using the variable number to increment and increments by the desired value. Plain just loads, adds, stores, quite long-windedly
                \item Foreach, both same length in-place, but Adv calls an different internal methods and has an extra method call, AND has additional gubbins for those methods.
            \end{itemize}
\newpage
\section{Conclusion}
