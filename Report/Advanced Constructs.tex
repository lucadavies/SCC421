...
\section{Results}
    ...
    \label{subsec:results}
        ...
        \subsubsection{Threats to Survey}
            The survey participants was comprised of a convenience sample of developers. This means that the results may not be generalised very easily to a wider group of developers. The results may be generalised \emph{within} ISS but not outside. Further research and more significant surveying should be undertaken to verify if the results found within ISS may be applied to a wider populace of developers.
        ...
        \subsubsection{Reasons To Use}
            ...
            It is not clear what advantages a `map' function has over a \codeword{foreach} loop to avoid off-by-one errors. In fact, conventional wisdom dictates that \codeword{foreach} loops, by nature should, avoid these errors by virtue of the fact that they\emph{remove} the need for a programmer to calculate the necessary number of loop iterations or use an index at all. It is plausible to suggest that this response was given by mistake, instead meaning to refer to traditional \codeword{for} loops, rather than \codeword{foreach} loops.
        ...
        \subsubsection{Reasons To Not Use}
            ...
            Only 4.5\% selected ``It would make code run slower'' - logically, this would make sense, factually too, as most of these constructs are plain syntactic differences that, with most modern compilers and interpreters, may produce the same bytecode and/or machine code. Lambdas and \codeword{foreach} loops are likely the only exceptions to this. The former has other potential implications beyond just how it looks on-screen, the latter (depending on language) may be a change from an array index to initialising an iterator and calling on that instead. This is reflected in the thoughts of participants in Figure \ref{fig:toNotUse}, showing that these two constructs were the only two to have received any selections pertaining to code run speed.

            The breakdown in Figure \ref{fig:toNotUse} also confirms that clarity was the most common reason selected, leadings by a wide margin for four of the constructs, but not for unary and compound operators (multiple reasons, described above) and for \codeword{foreach} loops (run speed was a concern as described immediately above).
        ...
        \subsubsection{Usage In Wider Industry}
            ...
            \begin{itemize}
                ...
                \begin{itemize}
                    \item the only construct that received a significant number of selections in any language a notable number of ``Below average'' selections, the other selections were also high giving a mean value corresponding to just marginally \emph{above} average for C\#, marginally \emph{below} average for JavaScript and just above the midpoint between below average and average for Java.
                \end{itemize}
    ...
    \subsection{Static Code Analysis}
        ...
        \subsubsection{Real-World Code Bases}
            []
        \subsubsection{SharpLab}
        \label{subsubsec:sharpLab}
            Fed .cs files into SharpLab (footnote: https://sharplab.io/) to get Roslyn CIL code.

            \begin{itemize}
                \item ~55 lines of gubbins to define lambda (probably, check) in Adv
                \item ternary: both `short-circuit' and exit at end of each branch (logic inverted in plain?)
                \item Lambda in Adv has much gubbins, beyond me, Roslyn/CIL expert better suited (looks like it creates a class instance and calls the lambda form within that class? Further evidence toward the fact that lambdas are more complicated than initially given credit for)
                \item Null-coalesce, not just 1-to-1 at all, Adv is much shorter, only one branch op
                \item Null-coalesce Adv doesn't declare variables, only uses stack (likely a context thing)
                \item Similar to coalesce, conditional doesn't declare locals (likely a context thing)
                \item Optimisation going on in conditional, missing literal, extra branch op
                \item increment Adv slightly LONGER somehow, even this isn't 1-to-1
                \item Compound assignment IS 1-to-1
                \item ?
            \end{itemize}
\newpage
\section{Conclusion}
